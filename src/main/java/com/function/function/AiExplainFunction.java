...
public class AiExplainFunction {
    ...
    private static final String JAVA_API_DICTIONARY =
        "【★★★ 檔案必須以這個檔頭開始：package 與下列 import 一個都不能少（缺 import 會 cannot find symbol）★★★】\n"
      ...
      + "  \n"
      + "  （CompanySalaryRule 與本類別同一個 package，不需 import；RuleUtils 一律用全名 com.function.util.RuleUtils.xxx，也不需 import。）\n"
      + "  類別宣告：public class Company{ID}Rule implements CompanySalaryRule { ... }\n\n"
    ...
    // =========================================================
    // JSON → Java 轉譯用（parse-rule 的規則 JSON 直接轉，不經 DRL）
    // 把 UpdateRulesFunction.buildDrl 的 action→RuleUtils 對應寫進 prompt
    // =========================================================
    private static final String JAVA_FROM_JSON_SYS =
        "你是 Drools/Java 專家。任務：把 parse-rule 解析出的『規則 JSON 陣列』，"
      + "等價轉譯成一個可直接編譯的 Java 類別，實作 com.function.function.SalaryRules.CompanySalaryRule。"
      + "語意必須與 JSON 規則完全相同。不需要、也不要先產生 DRL，直接輸出 Java。\n\n"

      + "【輸入 JSON 格式】一個陣列，每個元素是一條規則：\n"
      + "  { \"ruleName\":\"...\",\n"
      + "    \"conditions\":[ {\"field\":\"...\",\"operator\":\"...\",\"value\":...}, ... ],\n"
      + "    \"action\":\"...\",\n"
      + "    \"actionNote\":\"中文說明，只給人看、不影響計算\",\n"
      + "    \"actionWarning\":\"★真正被引擎拿去計算的參數，務必依此產生程式碼\" }\n\n"

      + "【action → Java 對應（action 大小寫不敏感；金額一律用下列 RuleUtils，不要自己改公式）】\n"
      + "1. addOvertimeBonus（加班費）：併入 processOvertimeBonus，並讓 hasCustomOvertimeCalc() 回 true。\n"
      + "   actionWarning 代表計算方法：\n"
      + "   (a) \"calcWeekdayOvertime\" / \"calcRestDayOvertime\" / \"calcNationalHolidayOvertime\" / \"calcStatutoryHolidayOvertime\"\n"
      + "       → BigDecimal bonus = com.function.util.RuleUtils.<方法名>(employee.getBaseSalary(), ot.getOvertimeHours());\n"
      + "   (b) \"calcWeekdayOvertimeByRate:<r1>:<r2>\"（自訂平日倍率，前2H=r1、超過2H=r2）\n"
      + "       → BigDecimal bonus = com.function.util.RuleUtils.calcWeekdayOvertimeByRate("
      + "employee.getBaseSalary(), ot.getOvertimeHours(), \"<r1>\", \"<r2>\");\n"
      + "   之後：result.setOvertimeBonus(result.getOvertimeBonus().add(bonus));\n"
      + "2. addLeaveDeduction（請假扣薪）：併入 processLeaveDeductions，並讓 hasCustomLeaveDeduction() 回 true。\n"
      + "   actionWarning 是扣薪比率字串（例 \"0.5\"；未填視為 \"1.0\"）：\n"
      + "   → BigDecimal deduct = com.function.util.RuleUtils.calcLeaveDeduction("
      + "employee.getBaseSalary(), lv.getLeaveHours(), \"<rate>\");\n"
      + "     result.setLeaveDeduction(result.getLeaveDeduction().add(deduct));\n"
      + "   processLeaveDeductions 需回傳 boolean（有處理到就回 true）。\n"
      + "3. addRuleDetail / addNote / addWarning：→ result.addRuleDetail(...) / result.addNote(...) / result.addWarning(...)。\n"
      + "4. fullAttendanceExempt：→ result.setFullAttendancePenaltyExempt(true)。\n\n"

      + "【conditions 處理】\n"
      + "- companyId == \"{ID}\" 這種公司守衛請【省略】（類別本身已是該公司專屬）。\n"
      + "- 針對 OvertimeFact 的條件（如 overtimeType == \"WEEKDAY\"）→ 變成 processOvertimeBonus 迴圈內的 if 守衛：\n"
      + "    for (OvertimeFact ot : overtimes) { if (\"WEEKDAY\".equals(ot.getOvertimeType())) { ...計算... } }\n"
      + "- 針對 LeaveFact 的條件（如 leaveTypeName == \"事假\"）→ 變成 processLeaveDeductions 迴圈內的 if 守衛：\n"
      + "    for (LeaveFact lv : leaves) { if (\"事假\".equals(lv.getLeaveTypeName())) { ...計算... } }\n"
      + "- EmployeeFact 的條件（如 baseSalary < 50000）→ 對應成 employee 上的 if 判斷。\n"
      + "- 走訪任何 List 前務必先做 null 防護。\n\n"

      + "【必做】\n"
      + "- 實作 3 個抽象方法：getCompanyId() 回 \"{ID}\"；getSeniorityMultiplier 回 1.0；"
      + "getCustomAllowances(3參數) 回 Collections.emptyMap()。\n"
      + "- 類別名 Company{ID}Rule，package com.function.function.SalaryRules。\n"
      + "- 只輸出純 Java，不要 markdown、不要 ``` 圍欄、不要解說。\n\n"
      + JAVA_API_DICTIONARY;
    ...
    // =========================================================
    // ④ 規則 → Java 轉譯（規則引擎規則 → 硬編碼對照組）
    //    POST /api/ai/generate-java
    //    Body（四選一，優先序：fix > rules > drl > blob）：
    //      { "companyId":"10", "rules":[ {parse-rule輸出的規則}... ] }  // ★不經DRL，直接由JSON轉譯
    //      { "companyId":"10", "drl":"<DRL內容>" }                      // 直接給 DRL 轉譯
    //      { "companyId":"10" }                                         // 伺服器查 Blob 的 DRL 後轉譯
    //      { "companyId":"10", "previousCode":"...", "compileError":"..." } // 修復編譯錯誤
    //    回應： { companyId, className, javaCode, sourceDrlChars, sourceMode }
    // =========================================================
    ...
    public HttpResponseMessage generateJava(
        ...
        try {
            ...
            // ★ rules 模式：parse-rule 的規則 JSON（陣列或單物件），若有帶就走「不經 DRL」直轉
            Object rulesObj  = reqBody.get("rules");
            String rulesJson = null;
            if (rulesObj != null) {
                String j = mapper.writeValueAsString(rulesObj);
                if (j != null && !j.isBlank()
                        && !j.equals("null") && !j.equals("[]") && !j.equals("{}")) {
                    rulesJson = j;
                }
            }
            ...
            if (!compileEr.isBlank() && !prevCode.isBlank()) {
                // 修復模式：把編譯錯誤丟回去要 AI 改（沿用 DRL 轉譯 system prompt 即可）
                sys = JAVA_TRANSPILE_SYS.replace("{ID}", companyId);
                sourceMode = "fix";
                userPrompt = "以下 Java 編譯失敗，請修正後重新輸出完整類別（只輸出純 Java）：\n\n"
                           + "=== 編譯錯誤 ===\n" + compileEr + "\n\n=== 目前程式碼 ===\n" + prevCode
                           + "\n\n提示：若錯誤是 cannot find symbol（例如 EmployeeFact/OvertimeFact/LeaveFact/SalaryResult），"
                           + "通常是缺 import，請務必補齊上面規定的必備 import 檔頭；package 必須是 com.function.function.SalaryRules。";
            } else if (rulesJson != null) {
                // ★ JSON 模式：直接用 parse-rule 的規則 JSON 轉 Java，完全不經 DRL
                sys = JAVA_FROM_JSON_SYS.replace("{ID}", companyId);
                sourceMode = "rules";
                srcChars = rulesJson.length();
                userPrompt = "公司 ID：" + companyId
                           + "\n以下是 parse-rule 解析出的規則 JSON，請據此輸出 Company"
                           + companyId + "Rule 的完整 Java 原始碼：\n\n" + rulesJson;
            } else {
                // DRL 模式：直接帶 drl，或未帶就用 companyId 去 Blob 查
                sys = JAVA_TRANSPILE_SYS.replace("{ID}", companyId);
                sourceMode = drl.isBlank() ? "blob" : "drl";
                if (drl.isBlank()) {
            ...
            resp.put("sourceMode", sourceMode);   // fix / rules / drl / blob
            return okResponse(request, mapper, resp);
